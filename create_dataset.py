...
@click.option('--input_image',
              type=click.STRING,
              default='datasets/cats/ther_60_80',
              help='The path to the input images.')
@click.option('--ground_truth',
              type=click.STRING,
              default='datasets/cats/thermal',
              help='The path to the ground truth images.')
@click.option('--edge_image',
              type=click.STRING,
              default='datasets/cats/rgb_edge_480_640',
...
def create_dataset(input_image, ground_truth, edge_image,
                   dataset_name, do_shuffle):
	list_a = create_list(input_image, True,
	                     datasets.DATASET_TO_IMAGETYPE[dataset_name], False)
	list_b = create_list(ground_truth, True, '.png')
	list_c = create_list(edge_image + '/rgb_edge_fused', True, '.png')
	list_d = create_list(edge_image + '/rgb_edge_out1', True, '.png')
	list_e = create_list(edge_image + '/rgb_edge_out2', True, '.png')
	list_f = create_list(edge_image + '/rgb_edge_out3', True, '.png')
	list_g = create_list(edge_image + '/rgb_edge_out4', True, '.png')
	list_h = create_list(edge_image + '/rgb_edge_out5', True, '.png')
